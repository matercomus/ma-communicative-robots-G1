...
class Ai2ThorClient:
    ...
    def search_for_object(self, objectType, event, controller):
        answer = ""
        found = self.search_for_object_in_view(objectType, event)
        rotate =0
        while not found and rotate<4:
            event = controller.step(action.R)
            found = self.search_for_object_in_view(objectType, event)
            rotate += 1
        if not found:
    ...
    def do_action(self, controller, w1, w2):
        answer = ""
        event = None
        found_objects = []
        try:
            event = controller.step(Action.RotateLeft.name)
            self._actions.append(Action.RotateLeft)
            if event:
                if w1.lower()=="find":
                    answer, found_objects = self.search_for_object(w2, event, controller)
                elif w1.lower()=="describe":
                    answer = self.what_do_you_see(event)
                elif w1.lower()=="move" or w1.lower()=="go":
                    if w2.lower()=="forward":
                        event = controller.step(Action.MoveAhead.name)
                        self._actions.append(Action.MoveAhead)
                    elif w2.lower()=="back":
                        event = controller.step(Action.MoveBack.name)
                        self._actions.append(Action.MoveAhead)
                    elif w2.lower()=="left":
                        event = controller.step(Action.RotateLeft.name)
                        self._actions.append(Action.RotateLeft)
                    elif w2.lower()=="right":
                        event = controller.step(Action.RotateRight.name)
    ...
    def process_instruction(self, event, prompt):
        self._answers =[]
        self._actions = []
        self._perceptions = []
        answer = ""
